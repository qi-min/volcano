...
func newTestBinder(t *testing.T, ctx context.Context) *testEnv {
	...
	client.AddWatchReactor("*", func(action k8stesting.Action) (handled bool, ret watch.Interface, err error) {
		...
		_watch, err := reactor.Watch(gvr, ns)
		if err != nil {
...
func (env *testEnv) initCSINodes(cachedCSINodes []*storagev1.CSINode) {
	...
	for _, csiNode := range cachedCSINodes {
		err := csiNodeInformer.GetIndexer().Add(csiNode)
		if err != nil {
			panic(err)
		}
	}
...
func (env *testEnv) addCSIDriver(csiDriver *storagev1.CSIDriver) {
	...
	err := csiDriverInformer.GetIndexer().Add(csiDriver)
	if err != nil {
		panic(err)
	}
}
...
func (env *testEnv) addCSIStorageCapacities(capacities []*storagev1beta1.CSIStorageCapacity) {
	...
	for _, capacity := range capacities {
		err := csiStorageCapacityInformer.GetIndexer().Add(capacity)
		if err != nil {
			panic(err)
		}
	}
...
// it should return v1beta1 version of CSIStorageCapacity to match csi backward compatibility
...
func TestFindPodVolumesWithoutProvisioning(t *testing.T) {
	...
	// changed csiDriver to v1beta1 for backward compatibility in volcano
